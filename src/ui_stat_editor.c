...
static const u16 evEnum[] = {
    MON_DATA_HP_EV, MON_DATA_ATK_EV, MON_DATA_DEF_EV, MON_DATA_SPATK_EV, MON_DATA_SPDEF_EV, MON_DATA_SPEED_EV
};

static void distributeUnusedEVs(void){
    u16 idx = 0;

    while(sStatEditorDataPtr->evUnused > 0){
        u16 currentEV = evEnum[idx];
        u16 currentEVValue = GetMonData(ReturnPartyMon(), currentEV);
        u16 canAllocate = EV_MAX_SINGLE_STAT - currentEVValue;

        // Dump them all
        if(canAllocate >= (sStatEditorDataPtr->evUnused)){
            u16 newValue = currentEVValue + sStatEditorDataPtr->evUnused;
            SetMonData(ReturnPartyMon(), currentEV, &newValue);
            sStatEditorDataPtr->evUnused = 0;
        }
        // Allocate what is possible
        else{
            u16 newValue = currentEVValue + canAllocate;
            SetMonData(ReturnPartyMon(), currentEV, &newValue);
            sStatEditorDataPtr->evUnused -= canAllocate;
        }
        ++idx;
    }
}
...
static void StatEditor_FreeResources(void){
    ...
    // ===== Distribute Unused EVs, so they are not lost on window close =======
    if(sStatEditorDataPtr->evUnused > 0){
        distributeUnusedEVs();
    }
...
// consider removal
static const u8 sText_MenuCurrentEvTotal[] = _("EVs");
...
static void PrintMonStats(){
    ...
    AddTextPrinterParameterized4(WINDOW_2, FONT_NARROW, 16, STARTING_Y + (DISTANCE_BETWEEN_STATS_Y * 6), 0, 0, sMenuWindowFontColors[FONT_WHITE], 0xFF, sText_MenuCurrentEvTotal);
    ...
    // ---- Total EVs ------
    ConvertIntToDecimalStringN(gStringVar2, sStatEditorDataPtr->evTotal, STR_CONV_MODE_RIGHT_ALIGN, 4);
    ...
    // ---- EVs Available to distribute -----
    ConvertIntToDecimalStringN(gStringVar2, sStatEditorDataPtr->evUnused, STR_CONV_MODE_RIGHT_ALIGN, 3);
    ...
    // ----- Get Ability -----
    int abilityNum = GetMonData(ReturnPartyMon(), MON_DATA_ABILITY_NUM);
    int abilityIndex = gSpeciesInfo[sStatEditorDataPtr->speciesID].abilities[abilityNum];
    StringCopy(gStringVar2, gAbilitiesInfo[abilityIndex].name);
...
static bool8 statCanIncrease(){
    ...
    else if(sStatEditorDataPtr->editingStat < IV_MAX_SINGLE_STAT){
        return TRUE;
    }
